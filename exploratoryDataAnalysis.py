...
def app():
    ...
    with st.expander("👉 Matches Per Season"):
       ...
       # st.write("matches_per_season data:", matches_per_season)
    ...
    #######################################################################    
    with st.expander('👉 Highest runs In IPL History By Players'):
        highest_scores = new_deliveriesDF.groupby('batter')['batsman_runs'].sum().sort_values(
            ascending=False
        ).head(20)
...
# Add new 'season' column to deliveries
    ...
    with st.expander('👉 Average Runs Scored By Teams In Last Over'):
        ...
        st.plotly_chart(fig, use_container_width=True, key=24)
    ...
    with st.expander('👉 Count of Matches Umpired By Different Umpires'):
        ...
        st.plotly_chart(fig, use_container_width=True, key=27)
